...
def mesher_and_interpolator(args):
    ...
    section_seq = CrossSectionSequence.from_file(args.infile_cross_sections, "Cross-sections",
    ...
    if args.infile_constraint_lines is None:
        lignes_contraintes = ConstraintLine.get_lines_and_set_limits_from_sections(section_seq,
                                                                                   args.interp_constraint_lines)
    else:
        lignes_contraintes = ConstraintLine.get_lines_from_file(args.infile_constraint_lines,
                                                                args.interp_constraint_lines)
        if args.nb_pts_lat is not None and len(lignes_contraintes) != 2:
        ...
        section_seq.find_and_add_limits(lignes_contraintes, args.dist_max)
...
parser.add_argument("--constant_long_disc", help="méthode de calcul du nombre de profils interpolés entre profils : "
                    "par profil (constant, ie True) ou par lit (variable, ie False)", action='store_true')
parser.add_argument("--interp_constraint_lines",
